...
clc
close all
paraini = 0.1;  %the coupling strength is changed in the test
para_after_change = [0.101,0.102,0.103,0.104,0.105,0.106];
...
for round = 1:max_round
    ...
    for i = 1:changepoint-1
        for j=1:M
            x(j,i+1)=x(j,i)+stepsize*(10*(y(j,i)-x(j,i))+paraini*adjmat0(j,:)*x(:,i));
    ...
    for i = changepoint:LL-1
        for j=1:M
            x(j,i+1)=x(j,i)+stepsize*(10*(y(j,i)-x(j,i))+para_after_change(round)*adjmat0(j,:)*x(:,i));
            y(j,i+1)=y(j,i)+stepsize*(28*x(j,i)-y(j,i)-x(j,i)*z(j,i));
            z(j,i+1)=z(j,i)+stepsize*(-8/3*z(j,i)+x(j,i)*y(j,i));
        end
    end
