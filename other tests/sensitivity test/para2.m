...
clc
close all
paraini = 28;   % rho is changed in the test
para_after_change = [28.2,28.4,28.6,28.8,29,29.2];
...
for round = 1:max_round
    ...
    for i = 1:changepoint-1
        for j=1:M
            ...
            y(j,i+1)=y(j,i)+stepsize*(paraini*x(j,i)-y(j,i)-x(j,i)*z(j,i));
    ...
    for i = changepoint:LL-1
        for j=1:M
            x(j,i+1)=x(j,i)+stepsize*(10*(y(j,i)-x(j,i))+C*adjmat0(j,:)*x(:,i));
            y(j,i+1)=y(j,i)+stepsize*(para_after_change(round)*x(j,i)-y(j,i)-x(j,i)*z(j,i));
            z(j,i+1)=z(j,i)+stepsize*(-8/3*z(j,i)+x(j,i)*y(j,i));
        end
    end
    for j=1:M
