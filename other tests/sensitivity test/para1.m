...
clc
close all
paraini = 10;  %sigma is changed in this test
para_after_change = [10.005,10.006,10.007,10.008,10.009,10.01];
...
for round = 1:max_round
    ...
    for i = 1:changepoint-1
        for j=1:M
            x(j,i+1)=x(j,i)+stepsize*(paraini*(y(j,i)-x(j,i))+C*adjmat0(j,:)*x(:,i));
    ...
    for i = changepoint:LL-1
        for j=1:M
            x(j,i+1)=x(j,i)+stepsize*(para_after_change(round)*(y(j,i)-x(j,i))+C*adjmat0(j,:)*x(:,i));
            y(j,i+1)=y(j,i)+stepsize*(28*x(j,i)-y(j,i)-x(j,i)*z(j,i));
            z(j,i+1)=z(j,i)+stepsize*(-8/3*z(j,i)+x(j,i)*y(j,i));
        end
    end
