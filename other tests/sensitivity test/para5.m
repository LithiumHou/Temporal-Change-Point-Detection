...
for round = 1:max_round
    ...
    z(:,1)=zini;
    X=zeros(3*M,LL);
    changepoint = 1780+20*round;   % the location of the change point is changed in the test
    stepsize=0.01;
    C = 0.1;
    ...
    for i = changepoint:LL-1
        for j=1:M
            x(j,i+1)=x(j,i)+stepsize*(10.1*(y(j,i)-x(j,i))+C*adjmat0(j,:)*x(:,i));
            y(j,i+1)=y(j,i)+stepsize*(28*x(j,i)-y(j,i)-x(j,i)*z(j,i));
            z(j,i+1)=z(j,i)+stepsize*(-8/3*z(j,i)+x(j,i)*y(j,i));
        end
    end
